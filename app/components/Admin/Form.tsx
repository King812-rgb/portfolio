...
export function Form({
  initialData,
  mode,
}: Readonly<{ initialData: Work | null; mode: "create" | "update" }>) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.screenshot || formData.screenshot.size === 0)
      newErrors.screenshot = "この項目は必須です";
    ...
    reader.onloadend = async () => {
      ...
      const url =
        mode === "create" ? "/api/works/createWork" : `/api/works/updateWork`;
      ...
      if (!result.ok) {
        alert(
          `${mode === "create" ? "作成" : "更新"}に失敗しました: ${result.status}`,
        );
        return;
      }
    ...
    <div className="text-center flex flex-col items-center">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col w-4/5 mx-auto">
        ...
        <div className="flex flex-col items-start m-5">
          ...
          />
          {errors.site_url && (
            <span className="text-red-500">{errors.site_url}</span>
          )}
        </div>
        ...
        <div className="flex flex-col items-start m-5">
          ...
          />
          {errors.github_url && (
            <span className="text-red-500">{errors.github_url}</span>
          )}
        </div>
        ...
        <div className="flex flex-col items-start m-5">
          ...
          />
          {errors.tech_stack && (
            <span className="text-red-500">{errors.tech_stack}</span>
          )}
        </div>
        ...
        <div className="flex flex-col items-start m-5">
          ...
          />
          {errors.screenshot && (
            <span className="text-red-500">{errors.screenshot}</span>
          )}
        </div>
        ...
        <div className="flex flex-col items-start m-5">
          ...
          />
          {errors.released_on && (
            <span className="text-red-500">{errors.released_on}</span>
          )}
        </div>
        ...
        <div className="flex items-center m-5">
          ...
          >
            <span className={`mr-3 font-semibold text-white`}>Publish</span>
            <div className="relative">
              ...
              />
              <div
                className={`block w-14 h-8 rounded-full transition-colors duration-300 ${formData.is_published ? "bg-green-500" : "bg-gray-300"}`}
              ></div>
              <div
